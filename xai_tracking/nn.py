...
VGG_types = {
    "VGG11": [64, "M", 128, "M"],
    "VGG13": [32, 32, "M", 64, 64, "M", 128, 128, "M", 256, 256, "M", 256, 512, "M"],
...
class WeirdBatchNorm2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            var =  ghost_sample.var((0,2,3), keepdim=True, unbiased=False)#.detach() # experimental detach to disconnect ghost sample from model and remaining batch
            sample_mean = sample.mean((0,2,3), keepdim=True)
            sample_var =  sample.var((0,2,3), keepdim=True, unbiased=False)#.detach() # experimental detach to disconnect ghost sample from model and remaining batch
        ...
        for x in architecture:
            if type(x) == int:
                ...
                layers += [
                    ...
                    WeirdBatchNorm2d(x, ghost_samples=self.ghost_samples, affine=False, momentum=0.1) if self.ghost_samples > 0 else torch.nn.BatchNorm2d(x, affine=False, momentum=0.1),
                    # nn.LayerNorm(x),
                    # nn.GroupNorm(32, x),
                    nn.ReLU(),
                ]
